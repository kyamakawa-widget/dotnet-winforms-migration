...
import {
  ...
  History, PlusCircle, RefreshCw, Download, Sparkles
} from "lucide-react"
                      ...
                      history.map(row => (
                        <tr key={row.orderNo} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-center">
                            <button onClick={() => handleDelete(row.orderNo)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-all" title="受注取消">
