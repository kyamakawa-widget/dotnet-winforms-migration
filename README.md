...
移行後は、責務に応じてコンポーネントを完全に分離し、Web 標準の技術スタックで再構築する。
...
本プロジェクトでは、単なるコード書き換えではなく、以下の 7 つの柱を軸に「開発プロセス」の移行を実践する。
...
> 「認証・認可 (Auth0 等)」「本番用 DB の冗長化構成」「ストリーミングレスポンス」「複数ターンの会話履歴管理」などは **対象外 (Out-of-Scope)**。
>
> **Agent の認証について**  
> Python Agent（`/chat`）は意図的に認証なしで実装。認証ロジックを Agent に直接書くことは技術的には可能だが（FastAPI の `Depends` による API キー検証等）、「推論サービスは推論のみに専念させる」という責務分離の方針により、認証は .NET API 側で一元管理する設計を想定。本番化する際の構成は React → .NET `/chat` `[Authorize]` → `localhost:8001/chat`。Agent は VPS 内部からのみ受け付ける前提。
